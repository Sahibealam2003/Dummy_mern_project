...
export const signup = async (req, res) => {
  try {
    ...
    if (existingUser) {
      console.log(
        "Signup Validation Failed: Email is already registered:",
        email,
      );
      return res.status(400).json({
...
export const verifyOTP = async (req, res) => {
  try {
    ...
    if (!userData) {
      return res.status(400).json({
        error:
          "Verification session expired or not found. Please sign up again.",
      });
    }
      ...
      try {
        ...
        if (decoded && decoded.exp) {
          ...
          if (remainingSeconds > 0) {
            ...
            console.log(
              `Token blacklisted in Redis for ${remainingSeconds} seconds`,
            );
          }
        }
      } catch (redisError) {
...
export const toggleWishlist = async (req, res) => {
  try {
    if (req.user.role === "admin") {
      return res
        .status(403)
        .json({ error: "Access denied. Admins cannot have a wishlist" });
    }
...
export const forgotPassword = async (req, res) => {
  try {
    ...
    try {
      ...
      await user.save();
      return res.status(500).json({
        error: "Failed to queue reset email. Please try again later.",
      });
    }
...
export const sendBroadcastEmail = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Emails list, subject, and message are required" });
    }
    ...
    res.status(200).json({
      success: true,
      message: `Successfully queued ${emails.length} emails for sending`,
    });
  } catch (error) {
...
export const sendCRMNotification = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ error: "Tokens list, title, and body are required" });
    }
    ...
    res.status(200).json({
      success: true,
      message: `Successfully sent ${fcmTokens.length} notifications`,
    });
  } catch (error) {
