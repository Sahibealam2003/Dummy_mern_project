...
const AdminPanel = () => {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            gsap.fromTo(".admin-tab-content-anim",
                { opacity: 0, y: 15 },
                { opacity: 1, y: 0, duration: 0.45, ease: "power2.out" }
            );

            if (activeTab === "crm") {
                gsap.fromTo(".crm-row-anim",
                    { opacity: 0, x: -10 },
                    { opacity: 1, x: 0, duration: 0.4, stagger: 0.04, ease: "power1.out", delay: 0.1 }
                );
            }
        }, adminPanelRef);
        return () => ctx.revert();
    ...
    useEffect(() => {
        if (productModalOpen || offerModalOpen || deleteModalOpen || crmModalOpen || crmMessageModalOpen) {
    ...
    }, [productModalOpen, offerModalOpen, deleteModalOpen, crmModalOpen, crmMessageModalOpen]);
    ...
    const handleUpdateUserRole = async (targetUser, newRole) => {
        try {
            ...
            if (currentCrmUser && (currentCrmUser.id === id || currentCrmUser._id === id)) {
    ...
    const openSendMessageModal = (target = "single", targetUser = null, targetSegment = "all") => {
        ...
        try {
            ...
            if (crmMessageTarget === "single") {
                ...
                const segmentUsers = users.filter(u => {
                    ...
                    const totalSpent = userOrders.reduce((sum, o) => o.orderStatus !== "Cancelled" ? sum + o.totalPrice : sum, 0);
                    const segment = totalSpent > 500 ? "vip" : (totalSpent >= 10 ? "regular" : "new");
            ...
            if (crmMessageType === "notification" || crmMessageType === "both") {
                if (targetFcmTokens.length === 0) {
                    if (crmMessageType === "notification") {
                        throw new Error("No Firebase FCM push notification tokens found for the selected recipients.");
                    }
                } else {
                    await sendCrmNotificationApi({
                        fcmTokens: targetFcmTokens,
                        title: crmMessageType === "both" ? crmMessageSubject : (crmMessageSubject || "Update from ShopX"),
                        body: crmMessageBody
                    });
                }
            }
            ...
            setError(err.message || err.response?.data?.error || "Failed to dispatch messages.");
        } finally {
    ...
    return (
        <div ref={adminPanelRef} className="flex h-screen bg-[#f5f3ef] text-[#2c2420] overflow-hidden">
            {/* Sidebar Desktop */}
            <aside className="hidden md:flex flex-col w-64 bg-white border-r border-[#ede8e2] shrink-0">
                ...
                <nav className="flex-1 px-4 py-6 space-y-1">
                    {menuItems.map((item) => {
                        ...
                        const active = currentPath === item.to || (item.to.includes("tab=") && location.search.includes(item.to.split("?")[1])) || (item.to === "/admin" && location.search === "");
                        return (
                            <Link
                                key={item.label}
                                to={item.to}
                                className={`flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold transition-all duration-200 ${active
            ...
            <div className="flex-1 flex flex-col min-w-0">
                ...
                <main className="flex-1 overflow-y-auto p-4 md:p-8">
                                                    ...
                                                    <tr key={order._id} className="hover:bg-stone-50/50 transition-colors group">
                                                        ...
                                                        <td className="p-4 text-center">
                                                            <select
                                                                ...
                                                                title={order.orderStatus === "Cancelled" || order.orderStatus === "Delivered" ? "Locked (Cancelled/Delivered orders cannot be modified)" : "Update Order Status"}
                                                                onChange={async (e) => {
                                                                ...
                                                                className={`rounded-lg border px-3 py-1.5 text-xs font-extrabold cursor-pointer focus:outline-none focus:ring-1 focus:ring-[#e8622a]/30 transition-all disabled:opacity-75 disabled:cursor-not-allowed ${order.orderStatus === "Pending"
                    ...
                    {activeTab === "crm" && (
                        <>
                            {usersLoading ? (
                                ...
                                <div className="text-center py-20 bg-white rounded-2xl border border-[#ede8e2] shadow-sm">
                                    <p className="text-sm font-semibold text-[#5a4e46]">No users found</p>
                                    <button onClick={fetchUsers} className="text-xs text-[#e8622a] font-bold hover:underline mt-2">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-4 rounded-2xl border border-[#ede8e2] shadow-sm">
                                        ...
                                        <div className="relative max-w-md w-full md:w-72">
                                            ...
                                            />
                                            <svg className="absolute left-3.5 top-3 h-3.5 w-3.5 text-[#8c7e74]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5">
                                                ...
                                                <tbody className="divide-y divide-[#f5f3ef]">
                                                    {(() => {
                                                        ...
                                                        if (filteredUsers.length === 0) {
                                                            return (
                                                                <tr>
                                                                    <td colSpan="7" className="p-8 text-center text-xs text-[#8c7e74] italic">
                                                        ...
                                                        return filteredUsers.map(u => {
                                                            ...
                                                            return (
                                                                <tr key={u._id} className="hover:bg-stone-50/50 transition-colors group">
                                                                    <td className="p-4 flex items-center gap-3">
                                                                        <div className="h-8 w-8 rounded-full overflow-hidden bg-[#e8622a]/10 border border-[#ede8e2] shrink-0">
                                                                            <img
                                                                                src={u.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(u.name)}&background=e8622a&color=fff&bold=true`}
                                                                                alt={u.name}
                                                                        ...
                                                                        <div className="overflow-hidden">
                                                                            <p className="text-xs font-bold text-[#2c2420] truncate">{u.name}</p>
                                                                            <p className="text-[10px] text-[#8c7e74] font-medium truncate">{u.email}</p>
                                                                        </div>
                                                                    </td>
                                                                    <td className="p-4">
                                                                        <span className={`inline-block rounded-full px-2 py-0.5 text-[9px] font-black uppercase tracking-wider border ${
                                                                            u.role === "admin"
                                                                                ? "bg-purple-50 text-purple-700 border-purple-200"
                                                                    ...
                                                                    <td className="p-4 text-center">
                                                                        <span className={`inline-block rounded-full px-2.5 py-0.5 text-[9px] font-extrabold uppercase tracking-wider ${
                                                                            segment === "VIP"
                                                                    ...
                                                                    <td className="p-4 text-center">
                                                                        <div className="flex items-center justify-center gap-2">
                                                                            ...
                                                                            <button
                                                                                onClick={() => openSendMessageModal("single", u)}
                                        ...
                                        {deleteType === "product" ? (
                                            ...
                                            <div className="w-full text-left flex items-center gap-3">
                                                <div className="h-10 w-10 rounded-full overflow-hidden bg-rose-100 flex items-center justify-center font-bold text-rose-700 uppercase shrink-0">
                                                    {deleteItem.avatar ? (
                                                        <img src={deleteItem.avatar} alt={deleteItem.name} className="h-full w-full object-cover" />
                                                    ) : (
                                                        deleteItem.name?.substring(0, 2)
                                                    )}
                                                </div>
                                                <div className="overflow-hidden">
                                                    <p className="text-xs font-bold text-[#2c2420] truncate">{deleteItem.name}</p>
                                                    <p className="text-[10px] text-[#8c7e74] font-medium truncate">{deleteItem.email}</p>
                                                </div>
                                            </div>
                                        )}
                                    </div>
                    ...
                    {crmModalOpen && currentCrmUser && (() => {
                        const userOrders = orders.filter(o => o.user?._id === currentCrmUser._id || o.user === currentCrmUser._id);
                        const totalSpent = userOrders.reduce((sum, o) => o.orderStatus !== "Cancelled" ? sum + o.totalPrice : sum, 0);
                        const orderCount = userOrders.length;
                        const segment = totalSpent > 500 ? "VIP Customer" : (totalSpent >= 10 ? "Regular Customer" : "New Customer");

                        return (
                            <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-md bg-[#2c2420]/45 animate-fade-in">
                                <div className="absolute inset-0 cursor-pointer" onClick={() => setCrmModalOpen(false)} />
                                ...
                                >
                                    ...
                                    <div className="flex flex-col sm:flex-row items-center gap-4 border-b border-[#ede8e2] pb-6 mb-6">
                                        <div className="h-16 w-16 rounded-full overflow-hidden border border-[#ede8e2] bg-[#e8622a]/10 shrink-0">
                                            <img
                                                src={currentCrmUser.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(currentCrmUser.name)}&background=e8622a&color=fff&bold=true`}
                                        ...
                                        <div className="text-center sm:text-left flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-[#8c7e74] font-medium truncate mt-0.5">{currentCrmUser.email}</p>
                                            <p className="text-[10px] text-stone-400 font-bold mt-1">Joined: {new Date(currentCrmUser.createdAt || Date.now()).toLocaleDateString()}</p>
                                        </div>
                                        <div className="flex flex-wrap justify-center gap-2 shrink-0">
                                            ...
                                            {currentCrmUser.role === "admin" ? (
                                                <button
                                                    onClick={() => handleUpdateUserRole(currentCrmUser, "user")}
                                                ...
                                                <button
                                                    onClick={() => handleUpdateUserRole(currentCrmUser, "admin")}
                                    ...
                                    <div className="grid grid-cols-3 gap-4 mb-6">
                                        <div className="bg-stone-50 border border-[#ede8e2]/60 rounded-xl p-3 text-center shadow-inner">
                                            <span className="text-[8px] font-black uppercase text-[#8c7e74] tracking-wider block">Total Spent</span>
                                            <span className="text-sm font-black text-[#2c7a4a] block mt-1">${totalSpent.toFixed(2)}</span>
                                        </div>
                                        <div className="bg-stone-50 border border-[#ede8e2]/60 rounded-xl p-3 text-center shadow-inner">
                                            <span className="text-[8px] font-black uppercase text-[#8c7e74] tracking-wider block">Orders Count</span>
                                            <span className="text-sm font-black text-[#2c2420] block mt-1">{orderCount}</span>
                                        </div>
                                        <div className="bg-stone-50 border border-[#ede8e2]/60 rounded-xl p-3 text-center shadow-inner">
                                            <span className="text-[8px] font-black uppercase text-[#8c7e74] tracking-wider block">Avg Order Value</span>
                                            ...
                                            <div className="border border-[#ede8e2] rounded-xl overflow-hidden shadow-sm max-h-60 overflow-y-auto">
                                                <table className="w-full text-left border-collapse text-xs">
                                                    ...
                                                    <tbody className="divide-y divide-[#f5f3ef] font-semibold text-[#5a4e46]">
                                                        {userOrders.map(ord => (
                                                            <tr key={ord._id} className="hover:bg-stone-50/20">
                                                                <td className="p-3 font-mono font-bold text-[#2c2420]">{ord.orderNumber}</td>
                                                                <td className="p-3 text-[#8c7e74] font-medium">
                                                                    {new Date(ord.createdAt).toLocaleDateString()}
                                                                </td>
                                                                <td className="p-3 text-right">
                                                                    {ord.orderItems.reduce((acc, it) => acc + it.quantity, 0)}
                                                                </td>
                                                                <td className="p-3 text-right font-black text-[#2c2420]">
                                                                    ${ord.totalPrice.toFixed(2)}
                                                                </td>
                                                                <td className="p-3 text-center">
                                                                    <span className={`inline-block rounded-full px-2 py-0.5 text-[8px] font-black border uppercase tracking-wider ${
                    ...
                    {crmMessageModalOpen && (
                        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-md bg-[#2c2420]/45 animate-fade-in">
                            <div className="absolute inset-0 cursor-pointer" onClick={() => !formLoading && setCrmMessageModalOpen(false)} />
                            ...
                            <div className="glass relative w-full max-w-lg rounded-2xl p-6 md:p-8 shadow-2xl animate-scale-in text-left">
                                ...
                                <h3 className="text-lg font-black text-[#2c2420] mb-1 flex items-center gap-1.5">
                                    ...
                                    {crmMessageTarget === "single" ? "Message Customer" : "Broadcast Marketing Campaign"}
                                </h3>
                                ...
                                <form onSubmit={handleSendMessageSubmit} className="space-y-4">
                                    {/* Channel Type */}
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-[10px] font-black uppercase tracking-wider text-[#8c7e74]">Message Channel</label>
                                        <div className="flex gap-4">
                                            {[
                                                { label: "Email (SMTP)", val: "email" },
                                                { label: "Push Notification (FCM)", val: "notification" },
                                                { label: "Both Channels", val: "both" }
                                            ].map(opt => (
                                                <label key={opt.val} className="flex items-center gap-2 text-xs font-bold text-[#5a4e46] cursor-pointer">
                                                    <input
                                                        type="radio"
                                                        name="crmMsgType"
                                                        value={opt.val}
                                                        checked={crmMessageType === opt.val}
                                                        onChange={e => setCrmMessageType(e.target.value)}
                                                        disabled={formLoading}
                                    ...
                                    {crmMessageTarget === "segment" && (
                                        <div className="flex flex-col gap-1.5">
                                            <label className="text-[10px] font-black uppercase tracking-wider text-[#8c7e74]">Recipient Customer Segment</label>
                                            <select
                                                value={crmMessageTargetSegment}
                                                onChange={e => setCrmMessageTargetSegment(e.target.value)}
                                                disabled={formLoading}
                                            ...
                                            >
                                                <option value="all">All Registered Customers ({users.length})</option>
                                                <option value="vip">VIP Customers (Spent &gt;$500)</option>
                                                <option value="regular">Regular Customers ($10-$500)</option>
                                    ...
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-[10px] font-black uppercase tracking-wider text-[#8c7e74]">
                                            {crmMessageType === "notification" ? "Push Notification Title" : "Email Subject"}
                                        </label>
                                        <input
                                            type="text"
                                            value={crmMessageSubject}
                                            onChange={e => setCrmMessageSubject(e.target.value)}
                                            placeholder={crmMessageType === "notification" ? "e.g. Flash Deal Live! ⚡" : "e.g. Special Discount Coupon for you!"}
                                            disabled={formLoading}
                                    ...
                                    {/* Message Body */}
                                    <div className="flex flex-col gap-1.5">
                                        <label className="text-[10px] font-black uppercase tracking-wider text-[#8c7e74]">Message Content / Body</label>
                                        <textarea
