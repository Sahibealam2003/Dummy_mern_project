...
const EmptyCartWarning = ({ onConfirm, onCancel }) => (
    <div className="mt-2 rounded-xl border border-rose-200 bg-rose-50 p-3 animate-scale-in">
...
const CartItem = ({ item }) => {
    ...
    return (
        <div
            className={`rounded-2xl border p-3 transition-all duration-200 ${showWarning
        ...
        >
            <div className="flex items-start gap-3">
                {/* Image */}
                <div className="flex h-16 w-16 shrink-0 items-center justify-center rounded-xl bg-white border border-[#ede8e2] p-1.5">
                    <img
                        src={item.image}
                        alt={item.title}
                ...
                <div className="flex flex-1 flex-col gap-1.5 min-w-0">
                    ...
                    <div className="relative">
                        ...
                        {showRemoveConfirm && (
                            <div className="absolute right-0 top-full mt-1 z-10 w-48 rounded-xl border border-rose-200 bg-white p-3 shadow-lg animate-scale-in">
                    ...
                    <div className="flex items-center justify-between mt-0.5">
                        ...
                        <div className="flex items-center gap-1">
                            <button
                                ...
                                className={`flex h-6 w-6 items-center justify-center rounded-lg border text-xs font-bold transition-all duration-150 active:scale-90 cursor-pointer ${item.quantity === 1
                            ...
                            <span className="flex h-6 w-7 items-center justify-center rounded-md border border-[#ede8e2] bg-[#fdfbf8] text-xs font-bold text-[#2c2420]">
                                {item.quantity}
                            </span>
                            <button
                                id={`cart-increment-${item.id}`}
                                onClick={() => dispatch(incrementQuantity(item.id))}
                                className="flex h-6 w-6 items-center justify-center rounded-lg border border-[#ede8e2] bg-white text-xs font-bold text-[#2c2420] transition-all duration-150 hover:border-[#e8622a]/50 hover:bg-[#fff3ed] hover:text-[#e8622a] active:scale-90 cursor-pointer"
                            >
                                +
                            </button>
                        </div>
...
const AuthPromptModal = ({ isOpen, onClose, onChooseOption }) => {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            {/* Modal Box */}
            <div className="relative w-full max-w-sm overflow-hidden rounded-2xl border border-[#ede8e2] bg-white p-6 shadow-2xl animate-scale-in z-10 text-center">
                {/* Close Button */}
                <button
                    onClick={onClose}
                ...
                <div className="mt-4 flex flex-col items-center">
                    ...
                    <div className="mt-6 w-full space-y-2.5">
                        <button
                            ...
                            className="btn-glow w-full rounded-full bg-gradient-to-r from-[#e8622a] to-[#c44e1e] py-2.5 text-xs font-bold uppercase tracking-wider text-white shadow-md shadow-[#e8622a]/20 transition-all cursor-pointer"
                        >
                            Sign In
                        </button>
                        <button
                            onClick={() => onChooseOption("signup")}
                            className="w-full rounded-full border border-[#ede8e2] bg-white py-2.5 text-xs font-bold uppercase tracking-wider text-[#5a4e46] transition-all hover:bg-stone-50 cursor-pointer"
                        >
                            Create Account
                        </button>
                        <button
                            onClick={onClose}
                            className="w-full rounded-full text-xs font-bold text-[#8c7e74] hover:text-[#2c2420] transition-colors py-1.5 cursor-pointer"
                        >
                            Cancel
                        </button>
                    </div>
            ...
            >
                ...
                <div className="flex items-center justify-between border-b border-[#ede8e2] px-5 py-4">
                    <div className="flex items-center gap-2.5">
                        <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-gradient-to-br from-[#e8622a] to-[#c44e1e] text-white shadow-lg shadow-[#e8622a]/20">
                    ...
                    <div className="flex items-center gap-2">
                        <button
                            ...
                            className="flex h-8 w-8 items-center justify-center rounded-full border border-[#ede8e2] bg-white text-[#8c7e74] transition-all duration-200 hover:rotate-90 hover:bg-[#fdf9f5] hover:text-[#2c2420] cursor-pointer"
                        >
                ...
                <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3">
                    {cartItems.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-24 text-center">
                            ...
                            <button
                                onClick={onClose}
                                className="mt-6 rounded-xl border border-[#e8622a]/30 bg-[#fff3ed] px-5 py-2 text-sm font-semibold text-[#e8622a] transition-all duration-200 hover:bg-[#ffe5d9] cursor-pointer"
                            >
                                Browse Products
                            </button>
                        </div>
                ...
                {cartItems.length > 0 && (
                    <div className="border-t border-[#ede8e2] px-5 py-4 space-y-3">
                        <div className="space-y-2">
                            ...
                            <div className="flex justify-between text-xs text-[#8c7e74]">
                                <span>Tax (10%)</span>
                                <span className="font-semibold text-[#2c2420]">${(totalPrice * 0.1).toFixed(2)}</span>
                            </div>
                        ...
                        <button
                            ...
                            className="btn-glow w-full rounded-xl bg-gradient-to-r from-[#e8622a] to-[#c44e1e] py-3 text-sm font-semibold text-white shadow-lg shadow-[#e8622a]/20 transition-all duration-200 hover:from-[#c44e1e] hover:to-[#a83c10] cursor-pointer"
                        >
                        ...
                        <button
                            onClick={onClose}
                            className="w-full rounded-xl border border-[#ede8e2] bg-transparent py-2.5 text-xs font-semibold text-[#8c7e74] transition-all duration-200 hover:border-[#d4c9be] hover:text-[#2c2420] cursor-pointer"
                        >
                            Continue Shopping
                        </button>
                    </div>
