...
const Navbar = ({ onCartOpen }) => {
    ...
    const [isNotifMuted, setIsNotifMuted] = useState(
    ...
    const handleEnableNotifications = async () => {
        ...
        // Otherwise request browser permission
        try {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            gsap.fromTo(".nav-logo-anim, .nav-link-anim",
                { y: -20, opacity: 0 },
                { y: 0, opacity: 1, duration: 0.6, stagger: 0.08, ease: "power2.out", delay: 0.15 }
            );
            gsap.fromTo(".nav-btn-anim",
                { scale: 0.85, opacity: 0 },
                { scale: 1, opacity: 1, duration: 0.5, stagger: 0.06, ease: "back.out(1.6)", delay: 0.45 }
            );
        }, navRef);
        return () => ctx.revert();
    }, []);
                ...
                <div className="mx-auto flex h-14 md:h-16 max-w-7xl items-center gap-4 md:gap-8 px-4 sm:px-6">
                        ...
                        {navItems.map(({ label, to, active, badge }) => {
                            ...
                            return (
                                <Component
                                    ...
                                    className={`nav-link-anim relative flex items-center gap-1.5 rounded-full px-4 py-2 text-sm font-semibold transition-all duration-200 select-none ${active
                    ...
                    <div className="ml-auto flex items-center gap-3">
                        ...
                        <div
                            ...
                            className="hidden lg:flex items-center gap-2 rounded-full border border-dashed border-[#e8622a]/30 bg-[#e8622a]/4 px-3.5 py-1.5 cursor-pointer transition-all duration-300 hover:scale-[1.02] hover:bg-[#e8622a]/8 hover:border-[#e8622a]/50 group nav-btn-anim"
                        >
                        ...
                        {isLoggedIn && (
                            <button
                                ...
                                className="relative flex h-10 w-10 items-center justify-center rounded-xl transition-all duration-200 hover:bg-[#e8622a]/8 hover:scale-105 active:scale-95 text-[#2c2420] hover:text-[#e8622a] cursor-pointer group nav-btn-anim"
                                aria-label="Notifications Settings"
                                title={
                                    notifPermission === "granted" && !isNotifMuted
                                        ? "Notifications are enabled (Click to mute)" 
                                        : "Notifications are disabled (Click to enable)"
                                }
                            >
                                {notifPermission === "granted" && !isNotifMuted ? (
                                    <svg className="h-5 w-5 text-[#e8622a] transition-all duration-200" viewBox="0 0 20 20" fill="currentColor">
                                        <path d="M10 2a6 6 0 00-6 6v3.586l-.707.707A1 1 0 004 14h12a1 1 0 00.707-1.707L16 11.586V8a6 6 0 00-6-6zM10 18a3 3 0 01-3-3h6a3 3 0 01-3 3z" />
                                    </svg>
                                ) : (
                                    <svg className="h-5 w-5 transition-colors duration-200" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.2">
                            ...
                            {isLoggedIn ? (
                                <>
                                    <button
                                        ...
                                        className={`nav-btn-anim flex items-center gap-2.5 rounded-lg border pl-1.5 pr-3.5 py-1 text-sm font-semibold transition-all duration-200 hover:scale-[1.02] active:scale-[95] cursor-pointer shadow-sm ${user?.role === "admin"
                                    ...
                                    >
                                        {user?.avatar ? (
                                            <img
                                                src={user.avatar}
                                                className={`h-8 w-8 rounded-full object-cover border bg-white ${user?.role === "admin" ? "border-amber-500 ring-2 ring-amber-400/20" : "border-stone-200/80"
                                                    }`}
                                                alt="avatar"
                                            />
                                        ) : (
                                ...
                                <>
                                    <button
                                        onClick={() => setDropdownOpen(!dropdownOpen)}
                                        className="flex items-center gap-2 rounded-lg border border-[#ede8e2] px-3.5 py-1.8 text-xs font-bold transition-all duration-200 hover:border-[#e8622a]/40 hover:bg-[#e8622a]/5 hover:scale-[1.02] active:scale-[95] cursor-pointer shadow-sm nav-btn-anim"
                                        style={{ color: "#2c2420" }}
                                    >
                                        <div
